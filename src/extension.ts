...
import { getScrcpyServerJar, startScrcpySession, ScrcpySession } from './scrcpy';

const WS_PORT = 9225;

const outputChannel = vscode.window.createOutputChannel('android-emu');

// Emulators we spawned (detached, headless). Killed when VS Code closes so they
// don't outlive the app. Emulators that were already running are NOT killed.
const spawnedEmulators: ChildProcess[] = [];
// Active mirror-session resources, killed on panel dispose and on deactivate.
let activeRecordProcess: ChildProcess | null = null;
// Low-latency scrcpy session (preferred video+input source); null when the
// mirror is running on the legacy screenrecord pipeline instead.
let activeScrcpySession: ScrcpySession | null = null;
let activeInputShell: ChildProcess | null = null;
let activeWss: WebSocketServer | null = null;
let activeSocket: WebSocket | null = null;
// Set by startMirror() so the rotation commands can reach the live session.
let rotateScreenFn: ((orientation: 'landscape' | 'portrait') => Promise<void>) | null = null;

// Fallback cleanup if deactivate() doesn't run (e.g. hard kill). Registered once.
process.on('exit', () => {
    for (const child of spawnedEmulators) { killProcessGroup(child); }
});
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const disposable = vscode.commands.registerCommand('android-emu.start', async () => {
        // Shared launch flow: doctor precheck, reuse a running emulator or boot an
        // AVD headless. Every failure path already shows an error message.
        const emu = await ensureEmulator();
        if (!emu) { return; }
        // Build the mirror panel for this serial.
    ...
    const rotateLandscapeCmd = vscode.commands.registerCommand('android-emu.rotateLandscape', () => {
        if (rotateScreenFn) { rotateScreenFn('landscape'); }
        else { vscode.window.showErrorMessage('android-emu: No active mirror session. Start the emulator first.'); }
    });
    ...
    const rotatePortraitCmd = vscode.commands.registerCommand('android-emu.rotatePortrait', () => {
        if (rotateScreenFn) { rotateScreenFn('portrait'); }
        else { vscode.window.showErrorMessage('android-emu: No active mirror session. Start the emulator first.'); }
    });
    context.subscriptions.push(rotatePortraitCmd);

    // Launch the current Expo project inside the emulator via Expo Go (the
    // prebuilt store app): boots/reuses the emulator, mirrors it, then starts
    // the Metro dev server in a terminal with `npx expo start --android`, which
    // also reverse-proxies port 8081 and opens the project in Expo Go.
    const startExpoGoCmd = vscode.commands.registerCommand('android-emu.startExpoGo', async () => {
        ...
        if (!emu) { return; }
        // The Expo CLI can open Expo Go but cannot install it; if it is missing,
        // still start the dev server so the user can install Expo Go from the
        // Play Store inside the emulator and then press "a" in the terminal.
    ...
    // Launch the current Expo project as a native development build: boots/reuses
    // the emulator, mirrors it, then runs `npx expo run:android` in a terminal,
    // which compiles the app with Gradle, installs the binary on the emulator and
...
export function deactivate() {
    rotateScreenFn = null;
    // Kill mirror-session children.
    try { activeScrcpySession?.stop(); } catch { /* ignore */ }
    activeScrcpySession = null;
    try { activeRecordProcess?.kill(); } catch { /* ignore */ }
    try { activeInputShell?.kill(); } catch { /* ignore */ }
    try { activeWss?.close(); } catch { /* ignore */ }
    // Kill detached headless emulators we spawned so they don't outlive VS Code.
...
// ---- Doctor ---------------------------------------------------------------
async function runDoctor(): Promise<{ ok: boolean; report: string; emulatorPath: string; adbPath: string }> {
    ...
    if (!androidHome) {
        ...
        emulatorPath = path.join(androidHome, 'emulator', 'emulator');
        if (!fs.existsSync(emulatorPath)) {
            failures.push('Emulator CLI not found at ' + emulatorPath);
        } else {
            const r = await runCommand(emulatorPath, ['-list-avds'], 10000);
            if (r.code !== 0) {
                failures.push('Emulator CLI did not run ("' + emulatorPath + ' -list-avds" failed).');
            }
        }
        const adbFull = path.join(androidHome, 'platform-tools', 'adb');
        if (fs.existsSync(adbFull)) {
            adbPath = adbFull;
...
async function getRunningEmulatorSerial(adbPath: string): Promise<string | null> {
    const r = await runCommand(adbPath, ['devices'], 5000);
...
async function waitForEmulatorSerial(adbPath: string, timeoutMs: number): Promise<string | null> {
...
// Shared launch flow used by the Start / Expo commands: doctor precheck, reuse a
// running emulator if one exists, otherwise pick + launch an AVD headless and
// wait for it to come online. Returns the adb serial + adb path, or null on
// failure (every failure path already shows the user an error message).
async function ensureEmulator(): Promise<{ serial: string; adbPath: string } | null> {
    // 1. Doctor precheck: scrcpy, emulator CLI, Android SDK (ANDROID_HOME).
    ...
    // 2. Use a running emulator if one exists; otherwise pick + launch an AVD headless.
    ...
    if (serial) {
        ...
        serial = await vscode.window.withProgress(
            ...
            () => waitForEmulatorSerial(doctor.adbPath, 120000)
        );
        if (!serial) {
...
// Like ensureEmulator(), but additionally waits for the FULL Android boot
// (`sys.boot_completed`), which app installs / activity starts require, and
// mirrors the headless emulator so the user can see and interact with it.
// Only starts a new mirror panel if none is live (the WS server binds a fixed
// port, so a second instance would fail to listen).
async function ensureBootedEmulator(context: vscode.ExtensionContext): Promise<{ serial: string; adbPath: string } | null> {
    const emu = await ensureEmulator();
    if (!emu) { return null; }
    ...
    const booted = await vscode.window.withProgress(
        ...
        () => waitForBootComplete(emu.adbPath, emu.serial, 180000)
    );
    if (!booted) {
    ...
    if (!activeWss) { startMirror(context, emu.serial, emu.adbPath); }
    return emu;
}

// adb lists an emulator as "device" once adbd is up, but installing packages and
// starting activities need the full boot (package manager, activity manager).
// Polls `getprop sys.boot_completed` until it reports 1.
async function waitForBootComplete(adbPath: string, serial: string, timeoutMs: number): Promise<boolean> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const r = await runCommand(adbPath, ['-s', serial, 'shell', 'getprop', 'sys.boot_completed'], 5000);
...
// Check whether an Android package id is installed on the device.
async function isPackageInstalled(adbPath: string, serial: string, pkg: string): Promise<boolean> {
    const r = await runCommand(adbPath, ['-s', serial, 'shell', 'pm', 'list', 'packages', pkg], 10000);
    return r.code === 0 && r.stdout.includes('package:' + pkg);
}

// Spawn the AVD headless and detached (the Node equivalent of trailing `&`).
//   $ANDROID_HOME/emulator/emulator -avd <name> -no-window -no-audio \
//       -gpu swangle -feature -Vulkan -no-boot-anim -no-snapshot-load \
//       -camera-back none -camera-front none
// On Linux (notably Fedora 44 / Mesa 26) the headless emulator segfaults (SEGV
// in qemu) during boot with the default -gpu auto. The crash is in Vulkan init
// + the virtual camera scene, so the fix is a combination verified against this
// machine:
//   -gpu swangle            -> ANGLE+SwiftShader software renderer (crash-safe)
//   -feature -Vulkan        -> disable Vulkan (the actual SIGSEGV site)
//   -camera-back/front none -> skip the mesh3d virtual scene init (also crashes)
//   -no-snapshot-load       -> skip any host-GPU snapshot (renderer mismatch =>
//                              fallback-to-scratch path also crashes). Clean
//                              cold boot instead; measured ~12s on this AVD.
// The process survives panel close; we kill it (by process group) when VS Code
// closes.
function launchEmulatorHeadless(emulatorPath: string, avd: string): Promise<ChildProcess | null> {
    return new Promise((resolve) => {
        ...
        // Give the spawn a moment to surface an ENOENT/'error' before resolving.
        setTimeout(() => resolve(child), 500);
    });
}

// ---- Mirror panel (webview + screenrecord + touch) ------------------------
function startMirror(context: vscode.ExtensionContext, serial: string, adbPath: string) {
    ...
    activeWss = wss;
    let stopped = false;
    // Set when we intentionally kill screenrecord to serve a reconnect; suppresses
    // the auto-restart in the exit handler so we don't spawn a duplicate process.
    let manualRestart = false;

    function startScreenRecord() {
        if (stopped) { return; }
        activeRecordProcess = spawn(adbPath, ['-s', serial, 'exec-out', 'screenrecord', '--output-format=h264', '-']);
        activeRecordProcess.stdout?.on('data', (chunk: Buffer) => {
            if (activeSocket && activeSocket.readyState === WebSocket.OPEN) {
                activeSocket.send(chunk, { binary: true });
            }
        });
        activeRecordProcess.stderr?.on('data', (d: Buffer) => outputChannel.appendLine('[screenrecord] ' + d.toString().trim()));
        activeRecordProcess.on('error', (err: Error) => outputChannel.appendLine('[screenrecord] ' + err.message));
        activeRecordProcess.on('exit', (code: number | null) => {
            outputChannel.appendLine('[screenrecord] exited code=' + code + ' ' + (stopped ? '(stopped)' : '(restarting)'));
            if (!stopped && !manualRestart) { setTimeout(startScreenRecord, 300); }
            manualRestart = false;
        });
    }

    function ensureInputShell(): ChildProcess | null {
        ...
        activeInputShell = spawn(adbPath, ['-s', serial, 'shell'], { stdio: ['pipe', 'pipe', 'pipe'] });
        activeInputShell.stdin?.setDefaultEncoding('utf-8');
        activeInputShell.stdout?.on('data', () => { /* ignore interactive shell output */ });
        activeInputShell.stderr?.on('data', (d: Buffer) => outputChannel.appendLine('[input-shell] ' + d.toString().trim()));
        activeInputShell.on('error', (err: Error) => outputChannel.appendLine('[input-shell] ' + err.message));
        activeInputShell.on('exit', (code: number | null) => {
    ...
    function sendInput(line: string) {
        ...
        if (sh?.stdin && !sh.stdin.destroyed) { sh.stdin.write(line + '\n'); }
    }

    // Launch primer: JMuxer only flushes a frame once the NEXT frame's start code
    // arrives. On a static screen screenrecord emits ~2 frames/8s, so without a
    // nudge the webview stays black until you tap. Open/close the notification
    // shade once to generate a frame burst, leaving the screen where it was.
    let primed = false;
    function primeDisplay() {
        if (primed || stopped) { return; }
        primed = true;
        setTimeout(() => {
            if (stopped) { return; }
    ...
    // ---- Video source selection (scrcpy preferred, screenrecord fallback) ----
    // scrcpy-server gives a tuned low-latency H.264 stream (~35-70ms
    // glass-to-glass, like the native scrcpy client) plus ~1ms input injection
    // over its control socket. screenrecord (the original pipeline, ~0.5-1s
    // video latency and ~100-300ms per input event) stays as a fallback for
    // when the server jar cannot be obtained (e.g. first run offline).
    const jarReady = getScrcpyServerJar(context.globalStorageUri.fsPath, (l) => outputChannel.appendLine(l));
    jarReady.then(j => outputChannel.appendLine(j
        ? '[scrcpy] low-latency video source enabled (scrcpy-server v2.4)'
        : '[scrcpy] server jar unavailable - using legacy screenrecord pipeline'));

    // Set when a scrcpy start failed once (bad jar / device incompatibility) so
    // we stop retrying it on every reconnect and go straight to screenrecord.
    ...
    async function startVideoSource() {
        ...
        if (stopped) { return; }
        if (jar) { startScrcpyVideo(jar); } else { startScreenRecord(); }
    }
    ...
    function startScrcpyVideo(jar: string) {
        startScrcpySession({
            adbPath, serial, jarPath: jar,
            log: (line: string) => outputChannel.appendLine(line)
        }).then(session => {
            if (stopped) { session.stop(); return; }
            activeScrcpySession = session;
            // Keep activeRecordProcess pointing at the server child too, so any
            // leftover direct .kill() (deactivate) also tears it down.
            ...
            // If the session comes up but no video bytes ever arrive (broken
            // option set, dead encoder), don't leave the user with a blank
            // panel: give up on scrcpy and fall back to screenrecord.
            setTimeout(() => {
            ...
            session.onExit(() => {
                if (activeScrcpySession === session) { activeScrcpySession = null; }
                if (activeRecordProcess === session.serverProcess) { activeRecordProcess = null; }
                if (!stopped && !manualRestart) { setTimeout(() => { void startVideoSource(); }, 300); }
                manualRestart = false;
            });
        }).catch(err => {
            outputChannel.appendLine('[scrcpy] failed to start (' + ((err as Error)?.message ?? err) + '); falling back to screenrecord');
            scrcpyBroken = true;
            startScreenRecord();
        });
    }
    ...
    (async () => {
        ...
        ensureInputShell();
        // Read the current display rotation so we can compute the correct viewport
        // dimensions and so the rotate commands know the starting point. We do NOT
        // force-lock here: on some devices (e.g. tablets whose panel is physically
        // mounted 180°) the default rotation 0 renders upside-down, and locking to
        // it would freeze that bad orientation. The rotate commands lock the
        // rotation only when the user intentionally rotates.
        let currentRotation = await queryRotation(adbPath, serial);
        outputChannel.appendLine('[mirror] current rotation: ' + currentRotation);

        // Display dimensions for a given rotation: odd rotations (90, 270)
        // swap width and height relative to the physical panel.
        function rotatedSize(rot: number): { width: number; height: number } {
            if (!physicalSize) { return { width: 0, height: 0 }; }
            return rot % 2 === 1
                ? { width: physicalSize.height, height: physicalSize.width }
                : { width: physicalSize.width, height: physicalSize.height };
        }
        ...
        wss.on('connection', (ws) => {
            ...
            // Always (re)start the video source on a new connection so the webview
            // receives a fresh SPS/PPS/IDR sequence. Neither the JMuxer/MSE path
            // nor the WebCodecs decoder can join an H.264 stream mid-flight.
            // When the webview is detached to a separate VS Code window the HTML
            // reloads and creates a new player; connecting it to an
            // already-running mid-stream feed would render black (video lost)
            // even though touch still works. Killing and relaunching the video
            // source gives the new page a clean stream.
            stopVideoSource();
            primed = false; // allow re-priming so the first frame flushes quickly
            void startVideoSource();
            primeDisplay();
            ws.on('message', (message: string) => {
                ...
                if (event.type === 'touch' && typeof event.action === 'string') {
                    ...
                    if (Number.isFinite(x) && Number.isFinite(y)) {
                        if (activeScrcpySession) {
                            // scrcpy control-socket injection: ~1ms, no per-event
                            // process spawn on the device (vs ~100-300ms for
                            // `input motionevent`). The screen size MUST be the
                            // size of the video the client is mirroring - the
                            // server drops the event otherwise. The webview
                            // reports its live display space (w/h); fall back to
                            // the host-tracked rotated size, then to `input
                            // motionevent` if neither is usable.
                            const dw = Number(event.w);
                            const dh = Number(event.h);
                            const sz = (Number.isFinite(dw) && dw > 0 && Number.isFinite(dh) && dh > 0)
                                ? { width: Math.round(dw), height: Math.round(dh) }
                                : rotatedSize(currentRotation);
            ...
            // Unlink this socket when it closes (e.g. the webview was detached to
            // a new window). The next connection will re-link activeSocket and
            // restart screenrecord above.
            ws.on('close', () => {
        ...
        // ---- Screen rotation ----------------------------------------------
        // Rotate the emulator display 90 degrees and update the webview frame.
        // The display rotation is locked (auto-rotate disabled) so the tracked
        // value is always authoritative. `input motionevent` coordinates are in
        // the current (rotated) display coordinate space, so the webview just
        // needs the swapped dimensions — no coordinate transform needed.
        rotateScreenFn = async (orientation: 'landscape' | 'portrait') => {
            if (stopped || !physicalSize) { return; }
            // Even rotations (0, 2) are landscape on this device's physical panel
            // shape; odd rotations (1, 3) are portrait. If we're already in the
            // requested orientation, do nothing.
            // Orientation from the rotation value, robust for any panel shape:
            // rotation swaps W/H on odd values, so compare against the physical
            // panel shape rather than assuming even == landscape.
            ...
            // Landscape and portrait alternate every 90 degrees, so exactly one
            // 90-degree step always crosses between them. The spin direction below
            // is only a visual choice for the CSS animation.
            const spinDirection: 'left' | 'right' = orientation === 'landscape' ? 'right' : 'left';
            currentRotation = spinDirection === 'right'
                ? (currentRotation - 1 + 4) % 4
                : (currentRotation + 1) % 4;
            // 1. Tell the webview to spin its viewport 90deg via CSS transform. This
            //    animates immediately (the old video rotates with the frame) so the
            //    user sees a smooth physical rotation instead of a janky dimension swap.
            if (activeSocket && activeSocket.readyState === WebSocket.OPEN) {
                activeSocket.send(JSON.stringify({ type: 'rotate', direction: spinDirection }));
            }
            // 2. SIMULTANEOUSLY tell the device to rotate underneath.
            ...
            // 3. Wait for the device to actually rotate before restarting capture.
            await new Promise(r => setTimeout(r, 2000));
            if (stopped) { return; }
            // 4. Kill the old stream, recreate the player (neither MSE nor a
            //    WebCodecs decoder reliably takes an H.264 SPS resolution change
            //    mid-stream), and start a fresh video source in the new
            //    orientation. The viewport is already in the new orientation
            //    from step 1, so the new stream just fills the existing frame.
            stopVideoSource();
            if (activeSocket && activeSocket.readyState === WebSocket.OPEN) {
        ...
        panel.onDidDispose(() => {
            ...
            activeWss = null;
            activeSocket = null;
            // Note: the headless emulator is intentionally NOT killed here so the
            // "use running emulator" flow works across panel reopens. It is killed
            // when VS Code closes (deactivate / process exit).
        }, null, context.subscriptions);
    })();
}

// Query the device display resolution via `adb shell wm size`.
// Output: "Physical size: 1080x2400" and, if a resolution override is active,
// "Override size: 768x1280". The override is the actually-rendered resolution.
function queryDeviceSize(adbPath: string, serial: string): Promise<{ width: number; height: number } | null> {
    return new Promise((resolve) => {
        const child = spawn(adbPath, ['-s', serial, 'shell', 'wm', 'size']);
        let out = '';
        let done = false;
        ...
        child.stderr?.on('data', (d: Buffer) => outputChannel.appendLine('[wm size] ' + d.toString().trim()));
        ...
        const timer = setTimeout(() => { child.kill(); finish(null); }, 3000);
    });
}

// Query the current display rotation via `cmd window user-rotation`.
// Output: "lock 0" or "free 1". Returns the numeric rotation (0-3).
async function queryRotation(adbPath: string, serial: string): Promise<number> {
    const r = await runCommand(adbPath, ['-s', serial, 'shell', 'cmd', 'window', 'user-rotation'], 5000);
...
// ---- Helpers --------------------------------------------------------------
function runCommand(cmd: string, args: string[], timeoutMs = 5000): Promise<{ code: number; stdout: string; stderr: string }> {
    return new Promise((resolve) => {
...
async function commandExists(cmd: string, args: string[]): Promise<boolean> {
    const r = await runCommand(cmd, args, 5000);
    return r.code === 0;
}

function killProcessGroup(child: ChildProcess) {
    if (!child.pid) { return; }
    try { process.kill(-child.pid, 'SIGKILL'); } catch { /* already dead or not permitted */ }
    try { child.kill('SIGKILL'); } catch { /* ignore */ }
}

// ---- Expo launch (Expo Go / native binary) ---------------------------------
// The workspace must look like an Expo project: either an app.json with an
// "expo" config block, or an "expo" dependency in package.json. Only the first
// workspace folder is considered (single-project workspaces).
function findExpoWorkspace(): vscode.WorkspaceFolder | null {
    ...
    } catch { /* malformed JSON etc. - fall through to the not-an-Expo-project error */ }
    if (!isExpo) {
...
// Run a command in a fresh integrated terminal rooted at the project so the
// dev server keeps running (and stays user-interactive, e.g. pressing "a").
function runInTerminal(name: string, cwd: string, command: string): vscode.Terminal {
...
// Save a base64-encoded PNG (from the webview canvas capture of the live <video>
// feed) to <workspace>/.android-emu/screenshots/screenshot_<timestamp>.png
async function writeScreenshot(dataUrl: string): Promise<void> {
    if (!dataUrl) {
    ...
    const dir = path.join(folder.uri.fsPath, '.android-emu', 'screenshots');
    fs.mkdirSync(dir, { recursive: true });
    const ts = new Date();
    const p = (n: number) => String(n).padStart(2, '0');
    const stamp = ts.getFullYear() + '-' + p(ts.getMonth() + 1) + '-' + p(ts.getDate()) +
        '_' + p(ts.getHours()) + '-' + p(ts.getMinutes()) + '-' + p(ts.getSeconds()) +
        '-' + String(ts.getMilliseconds()).padStart(3, '0');
    const file = path.join(dir, 'screenshot_' + stamp + '.png');
